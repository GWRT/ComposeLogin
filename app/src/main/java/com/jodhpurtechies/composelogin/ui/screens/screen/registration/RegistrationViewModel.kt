package com.jodhpurtechies.composelogin.ui.screens.screen.registration

import androidx.compose.runtime.mutableStateOf
import androidx.lifecycle.ViewModel
import com.jodhpurtechies.composelogin.ui.common.state.ErrorState
import com.jodhpurtechies.composelogin.ui.screens.screen.registration.state.*
...
class RegistrationViewModel : ViewModel() {
    ...
    fun onUiEvent(registrationUiEvent: RegistrationUiEvent) {
        when (registrationUiEvent) {
            ...
            is RegistrationUiEvent.NicknameChanged -> {
                registrationState.value = registrationState.value.copy(
                    nickname = registrationUiEvent.inputValue,
                    errorState = registrationState.value.errorState.copy(
                        nicknameErrorState = if (registrationUiEvent.inputValue.trim()
                                .isEmpty()
                        ) {
    ...
    private fun validateInputs(): Boolean {
        ...
        val mobileNumberString = registrationState.value.nickname.trim()
        ...
        return when {
            ...
            mobileNumberString.isEmpty() -> {
                registrationState.value = registrationState.value.copy(
                    errorState = RegistrationErrorState(
                        nicknameErrorState = mobileNumberEmptyErrorState
                    )
                )
                false
            }
